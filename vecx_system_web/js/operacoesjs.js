...
function mascaraCep(cep) {
    ...
    if (textoAtual.length === 10) {
        ...
        const parte2 = textoAtual.slice(5,7);
...
function textoAjustadoMascara(texto) {
    const textoAtual = texto.value;
    const textoSemHifen = textoAtual.replace(/\-/g, '');
    const textoSemBarra = textoSemHifen.replace(/\//g, '');
    const textoSemPonto = textoSemBarra.replace(/\./g, '');
    const textoSemParentese1 = textoSemPonto.replace(/\(/g, '');
    const textoSemParentese2 = textoSemParentese1.replace(/\)/g, '');
    texto.value = textoSemParentese2;
}
// onblur="mascaraCelular(this)" onfocus="textoAjustadoMascara(this)"